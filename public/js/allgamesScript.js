...
function populateGames(games, limit, pageNum) {
  let gamesUl = games
    .slice(limit * (pageNum - 1), limit * pageNum)
    .map((game) => {
      const imageHref = game.imageHref
        ? game.imageHref
      ...
      return `
        <div class="main__games-info">
            ...
            <div class="main__games-data">
                <a class="main__games-name" href="/games/${game.id}">${
        game.title
      }</a>
                <div class="main__ratings-stars-container">
                    <div class="main__ratings-stars">
                        <div class="main__ratings-stars-empty"></div>
                        <div class="main__ratings-stars-full" style="width:${
                          (game.overallRating / 5).toFixed(2) * 100
                        }%"></div>
                    </div>
                </div>
...
window.addEventListener("DOMContentLoaded", async (event) => {
  ...
  sidebarFilter.addEventListener("click", async (event) => {
    if (
      event.target.className.match(/main__sidebar-filter-(all|rating|console)$/)
    ) {
      let res = await fetch(`/games/api`, {
        method: "POST",
        body: JSON.stringify({
          filter: event.target.id,
          orderType: "overallRating",
        }),
        headers: {
          "Content-Type": "application/json",
        },
      ...
      const currFilter = document.querySelector(
        ".main__sidebar-filter--current"
      );
      const currOrder = document.querySelector(".main__sidebar-order--current");
      ...
      for (let i = 0; i < pageCount; i++) {
        if (i === 0) {
          pageNums.innerHTML += `<div class="main__games-page main__games-page--current" id="page${
            i + 1
          }">${i + 1}</div>`;
        } else {
          pageNums.innerHTML += `<div class="main__games-page" id="page${
            i + 1
          }">${i + 1}</div>`;
        }
      }
  ...
  pageNums.addEventListener("click", async (event) => {
    const currPage = document.querySelector(".main__games-page--current");
    const pageNum = event.target.id.match(/\d+/)
      ? parseInt(event.target.id.slice(-1))
      : null;

    if (!pageNum || currPage === event.target) {
      return;
    }
